...
def process_directory(directory):
    ...
    structured_data_prompt_path = resolve_file_path(data['analysisPlan']['structuredDataPrompt'])
    data['analysisPlan']['structuredDataPrompt'] = read_file(structured_data_prompt_path)

    # Recompose structuredDataSchema
    structured_data_schema_path = resolve_file_path(data['analysisPlan']['structuredDataSchema'])
    data['analysisPlan']['structuredDataSchema'] = json.loads(read_file(structured_data_schema_path))

    # Recompose successEvaluationPrompt
    success_evaluation_prompt_path = resolve_file_path(data['analysisPlan']['successEvaluationPrompt'])
    data['analysisPlan']['successEvaluationPrompt'] = read_file(success_evaluation_prompt_path)
...
def main():
    parser = argparse.ArgumentParser(description="Recompose Vapi assistant JSON files from extracted components.")
    parser.add_argument('directories', nargs='+', help='Path to one or more directories containing extracted assistant components')
